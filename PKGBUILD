...
_apply_patches() {
    ...
    # NOTE: this used to patch _registerShortcuts to call
    # this._registerWindowsShortcuts(view) on Linux, but that method doesn't
    # exist anywhere in the bundle (only the call site, no definition) - it
    # threw during window creation, so the app ran as a process but no window
    # ever appeared. The unpatched code path below already uses CmdOrCtrl
    # accelerators, which Electron maps to Ctrl on Linux/Windows, so no
    # Linux-specific branch is needed here at all.
